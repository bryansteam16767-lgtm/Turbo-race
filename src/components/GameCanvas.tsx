...
const GameScene = ({ 
  ...
  useFrame((state, delta) => {
    if (isSpectator) {
      const targetId = spectatedPlayerId;
      const targetPlayer = targetId ? players[targetId] : null;
      
      if (targetPlayer) {
        const dist = 40;
        const height = 20;
        const angle = targetPlayer.angle;
        
        const targetCamX = targetPlayer.x - Math.cos(angle) * dist;
        const targetCamZ = targetPlayer.y - Math.sin(angle) * dist;
        
        camera.position.lerp(new THREE.Vector3(targetCamX, height, targetCamZ), 0.1);
        camera.lookAt(targetPlayer.x, 0, targetPlayer.y);
      } else {
        // Default overview if no one to spectate
        camera.position.lerp(new THREE.Vector3(600, 400, 425), 0.05);
        camera.lookAt(600, 0, 425);
      }
      return;
    }
  ...
  return (
    <>
      ...
      {Object.values(players).map(p => {
        if (p.id === myId || p.isSpectator) return null;
        return (
...
export default function GameCanvas({ initialPlayers, onNewBestLap }: { initialPlayers?: Record<string, Player>, onNewBestLap?: (time: number) => void }) {
  ...
  const isSpectator = useMemo(() => {
    return myId ? players[myId]?.isSpectator : false;
  }, [myId, players]);

  // Auto-select first racer to spectate if we are a spectator
  useEffect(() => {
    if (isSpectator && !spectatedPlayerId) {
      const racers = Object.values(players).filter(p => !p.isSpectator);
      if (racers.length > 0) {
        setSpectatedPlayerId(racers[0].id);
      }
    }
  }, [isSpectator, players, spectatedPlayerId]);
  ...
  const handleNextSpectate = () => {
    ...
    const currentIndex = racers.findIndex(p => p.id === spectatedPlayerId);
    const nextIndex = (currentIndex + 1) % racers.length;
    setSpectatedPlayerId(racers[nextIndex].id);
  };
    ...
    <div className="relative w-full h-[850px] bg-slate-900 rounded-xl overflow-hidden shadow-2xl border-4 border-slate-700">
      ...
      {isSpectator && (
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4">
          <div className="bg-black/80 text-white px-6 py-3 rounded-xl border border-yellow-500/30 backdrop-blur-lg flex flex-col items-center shadow-2xl">
            <div className="text-[10px] text-yellow-500 uppercase tracking-[0.2em] font-black mb-1">Spectating</div>
            <div className="text-xl font-bold italic tracking-tight">
              {spectatedPlayerId ? players[spectatedPlayerId]?.name : 'No one racing'}
            </div>
          </div>
          <button 
            onClick={handleNextSpectate}
            className="bg-yellow-500 hover:bg-yellow-400 text-black font-black px-8 py-3 rounded-full shadow-lg transform transition-all active:scale-95 uppercase italic tracking-tighter"
          >
            Next Racer
          </button>
        </div>
